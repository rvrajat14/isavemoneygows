...
public class StatView: UIView {
    ...
    public override func draw(_ rect: CGRect) {
        ...
        var raduis: CGFloat = (rect.height/2) - CGFloat(self.tickness)
        
        if rect.height<rect.width {
            raduis = rect.height/2 - CGFloat(tickness)
        } else {
            raduis = rect.width/2 - CGFloat(tickness)
        }
        ...
        var color1:UIColor = UIColor(named: "progressLevelColor") ?? UIColor(red: 0.33, green: 0.41, blue: 0.79, alpha: 1.00)
...
//        circlePath.stroke()
        ...
        let mRect = CGRect(x: w*0.5 - size_amount.width*0.5,y: (h*0.85 - size_amount.height*0.70),width: (size_amount.width),height: size_amount.height)
        ...
        let fullCirclePath:UIBezierPath  = UIBezierPath(arcCenter: CGPoint(x: centerX, y: centerY), radius:
            CGFloat(raduis), startAngle: CGFloat(pi), endAngle: CGFloat(pi + angle*pi), clockwise: true)
